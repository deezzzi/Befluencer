...
@Component({
  ...
  imports: [],
  template: `
    <header class="h-12 sm:h-14 flex items-center justify-between px-4 sm:px-5 md:px-6 bg-white border-b border-slate-200 select-none">
  ...
  <h1 class="text-[14px] sm:text-[15px] font-semibold tracking-tight truncate">{{ currentTitle() }}</h1>
      </div>
  ...
  <div class="flex items-center gap-5 sm:gap-6 md:gap-8">
        ...
        <div id="bf-profile-anchor" class="flex items-center gap-3">
          <div class="h-8 w-8 sm:h-9 sm:w-9 rounded-full border border-slate-400 flex items-center justify-center text-slate-700 text-sm">{{ initials() }}</div>
          <div class="flex flex-col leading-tight">
            <span class="text-[13px] font-medium text-slate-900">{{ displayName() }}</span>
            <span class="text-[11px] text-slate-500 hidden sm:inline">Creator</span>
          </div>
